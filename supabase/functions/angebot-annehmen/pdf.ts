...
// --- Angebots-Dokument bei Direktbuchung (interne Rechnungsgrundlage) ---
const TXA = {
  de: {
    titel: (n: string) => `Angebot ${n}`,
    intro: (o: string) => `vielen Dank für Ihre Direktbuchung über unsere Website. Ihrer Buchung im ${o} liegt folgende Aufstellung zugrunde:`,
    zeitraum: (von: string, bis: string, n: number, p: number) => `Zeitraum: ${von} – ${bis} (${n} ${n === 1 ? "Nacht" : "Nächte"}) · ${p} ${p === 1 ? "Person" : "Personen"}`,
    gesamt: "Gesamtbetrag",
    hinweis: "Dieses Dokument wurde bei Ihrer Direktbuchung automatisch erstellt und dient als Grundlage der Rechnungen.",
  },
  en: {
    titel: (n: string) => `Offer ${n}`,
    intro: (o: string) => `thank you for booking directly via our website. Your booking at ${o} is based on the following breakdown:`,
    zeitraum: (von: string, bis: string, n: number, p: number) => `Period: ${von} – ${bis} (${n} ${n === 1 ? "night" : "nights"}) · ${p} ${p === 1 ? "person" : "people"}`,
    gesamt: "Total amount",
    hinweis: "This document was created automatically with your direct booking and serves as the basis for the invoices.",
  },
};

/** Baut den Inhalt des Angebots-Dokuments einer Direktbuchung (de/en). */
export function angebotInhalt(opts: {
  gastName: string; gastEmail: string; nummer: string; datumISO: string;
  anreiseISO: string; abreiseISO: string; naechte: number; personen: number;
  positionen: Position[]; gesamt: number; anbieter: Anbieter; lang?: PdfSprache;
}): DokumentInhalt {
  ...
  return {
    ...
    nummer: t.titel(opts.nummer),
    datumDE: datumL(opts.datumISO, lang),
    empfaenger: { name: opts.gastName, email: opts.gastEmail },
    intro: [
      t.intro(opts.anbieter.name),
      t.zeitraum(datumL(opts.anreiseISO, lang), datumL(opts.abreiseISO, lang), opts.naechte, opts.personen),
    ],
    positionen: opts.positionen,
    summen: [{ label: t.gesamt, betrag: opts.gesamt, fett: true }],
    hinweise: [t.hinweis],
