...
export interface PdfTexte {
  ...
  anzahlungFaellig: (datum: string) => string
  // Abschluss
...
const DE: PdfTexte = {
  ...
  anzahlungFaellig: (d) => `Die Anzahlung ist zahlbar innerhalb von 7 Tagen, bis spätestens ${d}. Erst mit Zahlungseingang ist Ihre Reservierung verbindlich und der Zeitraum fest für Sie geblockt.`,
...
const EN: PdfTexte = {
  ...
  anzahlungFaellig: (d) => `The deposit is payable within 7 days, by ${d} at the latest. Only once payment has been received is your reservation binding and the period firmly blocked for you.`,
...
const GR: PdfTexte = {
  ...
  anzahlungFaellig: (d) => `Η προκαταβολή είναι πληρωτέα εντός 7 ημερών, το αργότερο έως ${d}. Μόνο μετά την είσπραξη της πληρωμής η κράτησή σας είναι δεσμευτική και η περίοδος δεσμεύεται οριστικά για εσάς.`,
